...
from reactot.model import MLP, EGNN

class CondMLP(nn.Module):
    """
    FiLM conditioning MLP:
    Takes a scalar condition (e.g. solvent eps/eps_max)
    and outputs gamma and beta for feature modulation.
    """
    ...
    def forward(self, condition: torch.Tensor):
        """
        cond: (B, cond_dim)
        returns gamma, beta: (B, feature_dim)
        """
        return self.net(condition) # (B, cond_dim)
...
class BaseDynamics(nn.Module):
    def __init__(
        ...
        if source is not None and "model" in source:
            ...
            state_dict = source["model"]
            # Keep only parameters that exist in model and have matching shape
            filtered_dict = {k: v for k, v in state_dict.items() 
                            if k in model_dict and v.shape == model_dict[k].shape}

            # Print every key that was filtered out
            filtered_out_keys = [k for k in state_dict if k not in filtered_dict]
            missing_keys = [k for k in model_dict if k not in filtered_dict]
            
            if missing_keys:
                print(f"\nMissing keys in source (not loaded): {missing_keys}\n")
            if filtered_out_keys:
                print(f"\nFiltered out keys in source (not loaded due to missing or mismatched shape): {filtered_out_keys}\n")
