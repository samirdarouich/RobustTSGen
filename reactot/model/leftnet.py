...
from reactot.model.util_funcs import unsorted_segment_sum
from reactot.model.core import MLP
...
class EquiMessage(MessagePassing):
    ...
    def aggregate(
        ...
        x, vec = features
        reduce = "sum" if self.sum_aggr else "mean"
        x = scatter(x, index, dim=self.node_dim, dim_size=dim_size, reduce=reduce)
        vec = scatter(vec, index, dim=self.node_dim, dim_size=dim_size, reduce=reduce)
        return x, vec
...
class LEFTNet(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if condition_input_size > 0:
            self.condition_mlp = nn.Linear(condition_input_size, hidden_channels)
        else:
        ...
        for _ in range(num_layers):
            ...
            self.message_layers.append(
                EquiMessage(
                    hidden_channels, num_radial, reflect_equiv, sum_aggr=self.sum_aggr
                ).jittable()
            )
