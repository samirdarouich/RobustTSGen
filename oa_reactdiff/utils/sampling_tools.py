...
def write_single_xyz(xyzfile, natoms, out, charges):
    C2A = {
        1: "H",
        6: "C",
        7: "N",
    ...
    with open(xyzfile, "w") as fo:
        fo.write(str(natoms) + "\n\n")
        for ele, chrg in out, charges:
            ele = ele[: 3 + 5 + 1]
            chrg = chrg[:-1]
            x = ele[:3].cpu().numpy()
            _a = C2A[chrg[-1].long().item()]
            _x = " ".join([str(__x) for __x in x])
            fo.write(f"{_a} {_x}\n")


def write_tmp_xyz(
    fragments_nodes, out_samples, charges, idx=[0], prefix="gen", localpath="tmp", ex_ind=0
):
    TYPEMAP = {
        0: "react",
        1: "ts",
    ...
    for ii in idx:
        ...
        for jj, natoms in enumerate(fragments_nodes[0]):
            ...
            write_single_xyz(
                ...
                charges=charges[ii][start_ind:end_ind],
            )
            start_ind = end_ind
