...
def xh2pmg(xh,charges):
    mol = Molecule(
        species=charges[:, -1].long().cpu().numpy(),
...
def batch_rmsd(
    ...
    xh_use = xh[idx]
    charges_use = charges[idx]
    nodes = fragments_nodes[idx].long().cpu().numpy()
    start_ind, end_ind = 0, 0
    for jj, natoms in enumerate(nodes):
        end_ind += natoms
        mol1 = xh2pmg(out_samples_use[start_ind:end_ind], charges_use[start_ind:end_ind])
        mol2 = xh2pmg(xh_use[start_ind:end_ind], charges_use[start_ind:end_ind])
        try:
