import sys
sys.path.append("/mnt/beegfs/home/ac137577/projects/OAReactDiff")
import time
import os
import numpy as np
import torch
...
checkpoints = {
    ...
    "leftnet_new_encoder": "/mnt/beegfs/home/ac137577/projects/OAReactDiff/oa_reactdiff/trainer/checkpoint/OAReactDiff/leftnet-pretrained_no_en_decoder-95c4e836e498/last.ckpt",
}
...
if os.path.isfile(f"results/{filename}"):
    ...
    rmsds = []
TSEs = []
tmp_path = f"/mnt/beegfs/home/ac137577/projects/OAReactDiff/oa_reactdiff/evaluate/samples/{args.model}"
for num_repeat in range(config["repeats"]):
    ...
    for ii, batch in enumerate(loader):
        ...
        if ii == config["max_batch"]:
            break
        out_samples, xh_fixed, charges, fragments_nodes = inplaint_batch(
            batch,
            ddpm_trainer,
        ...
        save_path = f"{tmp_path}/repeat_{num_repeat}/batch_{ii}"
        os.makedirs(save_path, exist_ok=True)
        write_tmp_xyz(fragments_nodes, out_samples, charges, idx=[1], localpath=save_path)
        write_tmp_xyz(
            fragments_nodes, xh_fixed, charges, idx=[1], prefix="sample", localpath=save_path
        )
        _rmsds += batch_rmsd(
            fragments_nodes,
            out_samples,
        ...
        print(
            "rmsds: ", np.mean(_rmsds), np.std(_rmsds)
        )
    rmsds.append(_rmsds)
