...
from oa_reactdiff.dataset.embedding import atomic_fingerprint, row_one_hot, col_one_hot
...
ATOM_MAPPING.update({k: 5 for k in range(10, 100)})
ATOM_MAPPING.update({5: 5})  # add boron
...
class BaseDataset(Dataset):
    ...
    def process_molecules(
        ...
        if self.use_fingerprint_embedding:
            self.data[f"fingerprint_{idx}"] = [
                torch.tensor(
                    [
                        atomic_fingerprint(_at)
                        for _at in data["charges"][ii][: data["num_atoms"][ii]]
                    ],
                    device=self.device,
                )
