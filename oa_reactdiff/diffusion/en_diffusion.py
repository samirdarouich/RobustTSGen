...
from oa_reactdiff.model.embedding import NuclearEmbedding
...
class EnVariationalDiffusion(nn.Module):
    ...
    def __init__(
        ...
        if z_embedding_dim > 0:
            self.z_embedding = nn.Embedding(100, z_embedding_dim)  # assuming max atomic number is < 100
    ...
    def forward(
        ...
        if self.pos_only:
            z_t = [
                torch.cat([z_t[ii][:, : self.pos_dim], h0[ii]], dim=1) for ii in range(len(masks))
            ]
        ...
        # # In case not the original features are used, take just dummy 0 for the loss
        # if self.feature_mapping != FEATURE_MAPPING:
        #     loss_0_cat = [torch.zeros_like(loss) for loss in loss_0_cat]
        #     loss_0_charge = [torch.zeros_like(loss) for loss in loss_0_charge]
