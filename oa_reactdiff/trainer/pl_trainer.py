...
from oa_reactdiff.diffusion._normalizer import Normalizer
...
class DDPMModule(LightningModule):
    ...
    def compute_loss(self, batch):
        ...
        if self.loss_type == "l2" and self.training:
            ...
            #! should not be in a l2 loss
            loss_0 = torch.zeros_like(loss_t)
    ...
    def eval_inplaint_batch(
        ...
        xh_fixed = [
            torch.cat(
                [repre[feature_type] for feature_type in self.ddpm.feature_mapping],
                dim=1,
            )
        ...
        charges = [
            torch.cat(
                [repre[feature_type] for feature_type in ["charge"]],
                dim=1,
            )
            for repre in representations
        ]
